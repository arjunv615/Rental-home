...
?>
	...
	<table class="custom-profile-table">
		<tr>
			<th><?php _e('Month', 'text-domain'); ?></th>
			<th><?php _e('Payment Status', 'text-domain'); ?></th>
		</tr>
		<?php
		$months_data = get_user_meta($user->ID, 'months', true);
		foreach ($months_data as $month => $value) :
		?>
			<tr style="background-color: white; color: blue;">

				<td style="text-align:center;font-size:20px;"><?php _e($month, 'text-domain'); ?></td>
				<td style="text-align:center;"><?php echo $value == 0 ? "<h3 style='color:red;'>Unpaid</h3>" : "<h3 style='color:green;'>Paid</h3>" ?></td>
			</tr>
		<?php
		endforeach;
		?>
...
// add_action('show_user_profile', 'add_custom_user_profile_fields',1);
add_action('edit_user_profile', 'add_custom_user_profile_fields', 20);
...
?>
	...
	if (isset($_POST['user_dropdown']) && isset($_POST['month_dropdown'])) {
		...
		if ($user_data) {
			...
			$to = $email;
			$subject = 'Hello';
			$message = 'We have recived ' . $rent_month . ' rent payment successfully';
			...
			if ($result) {
	?>
				<div id="payment-status-wrapper" style="color:green;font-size:24px;margin-top:10px;"> <?php echo 'Email sent successfully.'; ?></div>
			<?php
			} else {
			?>
				<div id="payment-status-wrapper" style="color:red;font-size:24px;margin-top:10px;"> <?php echo 'Failed to send email.'; ?></div>
			<?php
			}
			// user meta update
			...
			if (array_key_exists($rent_month, $rent_paying_month)) {

				$rent_paying_month[$rent_month] = 1;
			...
			foreach ($months_data as $month => $value) {
				$payment_status = $value == 0 ? '<h3 style="color:red;">Unpaid</h3>' : '<h3 style="color:green;">Paid</h3>';
...
// Include the shortcodes.php file
// require_once get_template_directory() . '/shortcodes.php'; // Update with the correct path to shortcodes.php


function render_rental_shortcode()
{
	$months = array(
		'January', 'February', 'March', 'April', 'May', 'June',
		'July', 'August', 'September', 'October', 'November', 'December'
